...
export default function EstudiantesPage() {
  ...
  const [personaOption, setPersonaOption] = useState<"nueva" | "existente">(
    "nueva",
  );
  ...
  async function loadAll() {
    setLoading(true);
    const [e, c, es] = await Promise.all([
      getEstudiantes(),
      getCarreras(),
      getEstadosEstudiante(),
    ]);
  ...
  async function handleGuardar() {
    ...
    if (editando) {
      ...
      if (personaOption === "existente" && !form.idpersona) {
        showToast("Debes seleccionar una persona existente", "error");
        setSaving(false);
        return;
      }
      if (!form.correo || !form.clave || !form.expediente || !form.idcarrera) {
        showToast(
          "Correo, contraseña, expediente y carrera son obligatorios",
          "error",
        );
        setSaving(false);
        return;
      ...
      const studentRol = creation.rolesDisponibles.find(
        (rol) => rol.nombrerol === "ESTUDIANTE",
      );
      if (!studentRol) {
        showToast("No se encuentra el rol de estudiante", "error");
        setSaving(false);
        return;
      }

      const personaId = await creation.procesarStep1({
        opcion: personaOption,
        persona:
          personaOption === "nueva"
            ? {
            ...
            : undefined,
        idpersona: personaOption === "existente" ? form.idpersona : undefined,
      });
      if (!personaId) {
      ...
      const usuarioId = await creation.procesarStep2({
        correo: form.correo,
        clave: form.clave,
        idrol: studentRol.idrol,
      });
      if (!usuarioId) {
      ...
      const estudianteId = await creation.procesarStep3({
    ...
    if (res.success) {
      showToast(
        editando ? "Estudiante actualizado" : "Estudiante creado",
        "success",
      );
      setShowModal(false);
      loadAll();
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 text-center">
              <div className="text-2xl font-bold text-green-600">
                {loading ? "..." : estudiantes.filter((e) => e.activo).length}
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  exportToCSV(filtered, "estudiantes");
                      ...
                      filtered.map((e) => (
                        <tr key={e.idestudiante} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-center">
                            <span
                              className={`badge-${e.activo ? "success" : "error"}`}
                            >
                          ...
                          <td className="px-4 py-3 text-center space-x-1">
                            ...
                            <button
                              ...
                              className={`text-xs px-2 py-1 rounded ${e.activo ? "btn-outline" : "btn-ieproes"}`}
                            >
                              {e.activo ? "🚫" : "✅"}
                            </button>
                          </td>
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">
                {editando ? "✏️ Editar Estudiante" : "➕ Nuevo Estudiante"}
              </h2>
              <button
                onClick={() => setShowModal(false)}
            ...
            {editando ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      className="input-ieproes"
                      value={editForm.idestado}
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                          idestado: parseInt(e.target.value),
                        })
                      }
                    >
                      <option value={0}>Sin cambiar</option>
              ...
              <div className="space-y-3">
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      ¿Crear nueva persona o usar una existente?
                    </label>
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          name="personaOption"
                          value="nueva"
                          checked={personaOption === "nueva"}
                          onChange={() => {
                            setPersonaOption("nueva");
                            setForm({ ...form, idpersona: 0 });
                          }}
                        />
                        Nueva persona
                      </label>
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          name="personaOption"
                          value="existente"
                          checked={personaOption === "existente"}
                          onChange={() => {
                            setPersonaOption("existente");
                            setForm({
                              ...form,
                              primernombre: "",
                              primerapellido: "",
                ...
                {personaOption === "existente" ? (
                  <div>
                    ...
                    >
                      ...
                      {creation.personasDisponibles.map((persona) => (
                        <option
