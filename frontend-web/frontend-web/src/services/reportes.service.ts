...
export async function crearMateria(payload: {
  ...
  try {
    ...
    return {
      success: false,
      message: e.response?.data?.message || "Error al crear materia",
    };
  }
}
...
export async function editarMateria(
  ...
  try {
    ...
    return {
      success: false,
      message: e.response?.data?.message || "Error al editar materia",
    };
  }
}

export async function toggleMateriaActive(id: number) {
  try {
    const { data } = await api.patch(`/api/materias/${id}/toggle-active`);
    return data;
  } catch (e: any) {
    return {
      success: false,
      message:
        e.response?.data?.message || "Error al cambiar estado de materia",
    };
  }
}
...
export async function crearGrupo(payload: {
  ...
  try {
    ...
    return {
      success: false,
      message: e.response?.data?.message || "Error al crear grupo",
    };
  }
}
