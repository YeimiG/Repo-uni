===Borrar todo y reiniciar desde 1 las tablas===

TRUNCATE 
    seguridad.RolPermiso, seguridad.Permiso, seguridad.Modulo, seguridad.Usuario, seguridad.Rol,
    academico.PlanEstudio, academico.Carrera, academico.Escuela, academico.Facultad,
    estudiantes.Estudiante, estudiantes.EstadoEstudiante,
    personas.Persona, personas.Municipio, personas.Departamento
...
-- SUPER_ADMIN (ID 1)
-- Le asignamos ABSOLUTAMENTE TODOS los permisos existentes
INSERT INTO seguridad.RolPermiso (idRol, idPermiso)
SELECT 1, idPermiso FROM seguridad.Permiso;

-- ADMIN_ACADEMICO (ID 2)
-- Gestiona Carreras, Materias, Planes, Estudiantes y Docentes
...
-- COORDINADOR (ID 4)
-- Grupos, Horarios e Inscripciones
INSERT INTO seguridad.RolPermiso (idRol, idPermiso) VALUES
(4, 1),  -- Ver Dashboard
...
-- ESTUDIANTE (ID 6)
-- Solo consulta de sus propios datos
INSERT INTO seguridad.RolPermiso (idRol, idPermiso) VALUES
(6, 1),  -- Ver Dashboard
(6, 11), -- Ver materias
...
-- SECRETARIA (ID 8)
-- Apoyo en inscripciones y visualización básica
INSERT INTO seguridad.RolPermiso (idRol, idPermiso) VALUES
(8, 1),  -- Ver Dashboard
...
-- Script Unificado con los 14 Departamentos
-- ==============================================
...
-- Facultad de Ingeniería y Arquitectura (idFacultad = 1)
...
-- Facultad de Ciencias Económicas (idFacultad = 2)
...
-- Escuela de Ingeniería de Sistemas (idEscuela = 1)
...
-- Direcciones en San Salvador (idMunicipio suele empezar en 40 según el orden unificado previo)
...
-- Direcciones en Santa Tecla (La Libertad - idMunicipio 21 aprox)
...
-- Direcciones en Santa Ana (idMunicipio 51 aprox)
...
-- Direcciones en San Miguel (idMunicipio 34 aprox)
('Colonia Ciudad Jardín', 'Calle Los Pinos #5', 'A espaldas del estado Charlaix', '03101', 34, true),

-- Direcciones en Ahuachapán (idMunicipio 1 aprox)
...
-- Juan Pérez (Usa DUI=1, Dirección=1, Contacto=1)
('Juan', 'Alberto', 'Pérez', 'Rodríguez', 
 '1995-05-15', 'M', 'SOLTERO', 1, '05123456-7', 1, 1, true),

-- María López (Usa DUI=1, Dirección=2, Contacto=2)
('María', 'Elena', 'López', 'García', 
 '1998-10-22', 'F', 'SOLTERO', 1, '06123456-8', 2, 2, true),

-- Carlos Rivas (Usa Pasaporte=2, Dirección=3, Contacto=3)
('Carlos', 'Mauricio', 'Rivas', 'Zaldaña', 
 '1985-03-12', 'M', 'CASADO', 2, 'A12345678', 3, 3, true),

-- Ana Estrada (Usa DUI=1, Dirección=4, Contacto=4)
('Ana', 'Beatriz', 'Estrada', 'Pineda', 
 '2000-01-30', 'F', 'SOLTERO', 1, '07123456-9', 4, 4, true);

-- ==============================================
-- INSERTAR USUARIOS (TODOS LOS CAMPOS - SIN NULL)
-- SCHEMA: SEGURIDAD
-- ==============================================
...
-- 2. Administrador Académico (idRol 2)
...
-- ==============================================
-- INSERTAR PLANES DE ESTUDIO (TODOS LOS CAMPOS)
-- SCHEMA: ACADEMICO
-- ==============================================
...
-- SCHEMA: ESTUDIANTES (O CATALOGOS)
-- ==============================================

INSERT INTO estudiantes.Estado (
    idestado, 
    nombre, 
    descripcion, 
    activo
) VALUES
-- 1. ACTIVO: El estado base que usamos en los inserts anteriores
(1, 'ACTIVO', 'Estudiante con matrícula vigente y apto para inscribir materias', true),

-- 2. INACTIVO: Para quienes no se matricularon en el ciclo actual
(2, 'INACTIVO', 'Estudiante que no ha registrado carga académica en el periodo vigente', true),

-- 3. EGRESADO: Finalizó materias pero no se ha graduado
(3, 'EGRESADO', 'Estudiante que ha completado el 100% de su plan de estudios', true),

-- 4. GRADUADO: Ya recibió su título
(4, 'GRADUADO', 'Persona que ha recibido su título profesional', true),

-- 5. SUSPENDIDO: Por motivos disciplinarios o administrativos
(5, 'SUSPENDIDO', 'Estudiante con restricción temporal de acceso al sistema', true),

-- 6. RETIRADO: El estudiante solicitó su baja voluntaria
...
-- Estudiante 1: Ana Estrada (idPersona 4, idUsuario 4)
(
    4,                    -- idpersona
    'EXP-2026-001',       -- expediente
    CURRENT_DATE,         -- fechaingreso
    1,                    -- idcarrera (Ing. Sistemas)
    1,                    -- idplanestudio (Plan 1)
    1,                    -- idestado (Activo)
    1,                    -- idtipoingreso (Nuevo Ingreso)
    'Instituto Nacional de San Salvador', -- colegioprocedencia
    2025,                 -- añoegresocolegio
    8.5,                  -- promedioingreso
    0,                    -- cantidadmateriasaprobadas
    0,                    -- cantidadmateriasreprobadas
    0.0,                  -- indiceglobal
    0.0,                  -- indiceperiodo
    0,                    -- creditosacumulados
    160,                  -- creditostotales (Total de la carrera)
    0.0,                  -- porcentajeavance
    4,                    -- idusuario
    true                  -- activo
),

-- Estudiante 2: Maria Lopez (idPersona 2, idUsuario 2)
(
    2,                    -- idpersona
    'EXP-2026-002',       -- expediente
    CURRENT_DATE,         -- fechaingreso
    4,                    -- idcarrera (Lic. Administración)
    4,                    -- idplanestudio (Plan 4)
    1,                    -- idestado (Activo)
    1,                    -- idtipoingreso (Nuevo Ingreso)
    'Colegio García Flamenco', -- colegioprocedencia
    2025,                 -- añoegresocolegio
    9.0,                  -- promedioingreso
    0,                    -- cantidadmateriasaprobadas
    0,                    -- cantidadmateriasreprobadas
