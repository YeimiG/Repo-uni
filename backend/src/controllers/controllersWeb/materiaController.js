...
exports.getMaterias = async (req, res) => {
  try {
    // Admin ve todas las materias (incluyendo inactivas) para poder activarlas/desactivarlas
    const result = await db.query(
      "SELECT * FROM academico.materia ORDER BY codigo",
    );
...
exports.crearMateria = async (req, res) => {
  ...
  try {
    const existe = await db.query(
      "SELECT idmateria FROM academico.materia WHERE codigo = $1",
      [codigo],
    );
    ...
    const result = await db.query(
      ...
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING idmateria
    `,
      [
        codigo,
        nombre,
        ...
        horaspracticas || 0,
        tipo || "OBLIGATORIA",
        req.body.activo === undefined ? true : req.body.activo,
      ],
    );
    ...
    res.json({
      ...
      idmateria: result.rows[0].idmateria,
    });
  } catch (error) {
...
exports.editarMateria = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al editar materia" });
  }
};

exports.toggleMateriaActive = async (req, res) => {
  const { id } = req.params;
  try {
    // Obtener estado actual
    const resultado = await db.query(
      "SELECT activo FROM academico.materia WHERE idmateria = $1",
      [id],
    );
    ...
    // Actualizar estado
    await db.query(
      "UPDATE academico.materia SET activo = $1 WHERE idmateria = $2",
      [nuevoEstado, id],
    );

    res.json({
      success: true,
      message: `Materia ${nuevoEstado ? "activada" : "desactivada"} correctamente`,
      activo: nuevoEstado,
    });
  } catch (error) {
    console.error("ERROR TOGGLE MATERIA:", error);
    res
      .status(500)
      .json({ success: false, message: "Error al cambiar estado de materia" });
  }
};
  ...
  try {
    ...
    if (existe.rows.length > 0)
      return res.status(400).json({
        success: false,
        message: "Ya existe ese grupo en este período",
      });
